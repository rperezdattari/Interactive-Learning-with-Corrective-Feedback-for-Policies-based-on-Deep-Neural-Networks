...
def get_reward(name, res, type, folder_name):
    ...
    if folder_name == 'DDPG':
        x_raw = np.load('results/Cartpole-v0/%s/timestep_%s.npy' % (folder_name, name)) * (1/22.5)  # to seconda
        x = []
        x.append(x_raw[0])
        ...
        x_raw = np.load('results/Cartpole-v0/%s/time_%s.npy' % (folder_name, name)) * (1 / 22.5)  # to seconds
        x = x_raw
        x = np.array(x) / 60  # to minutes
        f = interp1d(np.append(0, x), np.append(0, reward_results))
        x = np.arange(0, x[-1], res)
