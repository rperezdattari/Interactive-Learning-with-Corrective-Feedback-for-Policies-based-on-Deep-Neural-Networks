...
def get_reward(name, res, type, folder_name):
    reward_results = np.load('results/Cartpole-v0/%s/reward_%s.npy' % (folder_name, name))

    if folder_name == 'NN_teacher_Perr' or folder_name == 'NN_teacher_Perr_nBuff':
        x_raw = np.load('results/Cartpole-v0/%s/time_%s.npy' % (folder_name, name)) * (1/20.5)  # to seconds
        x = x_raw
        ...
        x_raw = np.load('results/Cartpole-v0/%s/timestep_%s.npy' % (folder_name, name)) * (1/20.5)  # to seconda
        x = []
        x.append(x_raw[0])
        ...
        x = np.load('results/Cartpole-v0/%s/time_%s.npy' % (folder_name, name)) / 60.0
        f = interp1d(np.append(0, x), np.append(0, reward_results))
        x = np.arange(0, x[-1], res)
        reward_results = f(x)
    else:
...
def plot_reward(reward_results, episodes, name, type=None, var_rewards=None, color=None, alpha=None, linestyle='-'):
    ...
    if type == 'time':
        ...
        plt.xlabel('Episode')
    plt.title('D-COACH Experience Replay Buffer Comparison: Cart-Pole Training')
...
mean_plot(experiments4, plot_type, 'NN_teacher_Perr', 'Buffer; error prob. 0%', color='C0') # 0%  err
mean_plot(experiments1, plot_type, 'NN_teacher_Perr_nBuff', 'No buffer; error prob. 0%', color='C0', linestyle='--') # 0%  err

mean_plot(experiments1, plot_type, 'NN_teacher_Perr', 'Buffer; error prob. 10%', color='C1') # 10%  err
mean_plot(experiments2, plot_type, 'NN_teacher_Perr_nBuff', 'No buffer; error prob. 10%', color='C1', linestyle='--') # 10%  err

mean_plot(experiments2, plot_type, 'NN_teacher_Perr', 'Buffer; error prob. 20%', color='C2') # 20%  err
mean_plot(experiments3, plot_type, 'NN_teacher_Perr_nBuff', 'No buffer; error prob. 20%', color='C2', linestyle='--') # 20%  err

mean_plot(experiments3, plot_type, 'NN_teacher_Perr', 'Buffer; error prob. 30%', color='C3') # 30% err
mean_plot(experiments4, plot_type, 'NN_teacher_Perr_nBuff', 'No buffer; error prob. 30%', color='C3', linestyle='--') # 30%  err
