...
def get_reward(name, res, type, folder_name):
    ...
    if folder_name == 'NN_teacher_Perr' or folder_name == 'NN_teacher_Perr_nBuff':
        x_raw = np.load('../CartPole_OpenAI/results/%s/time_%s.npy' % (folder_name, name)) * (1/20.5)  # to seconds
        x = x_raw
        ...
        x_raw = np.load('../CartPole_OpenAI/results/%s/timestep_%s.npy' % (folder_name, name)) * (1/20.5)  # to seconda
        x = []
        x.append(x_raw[0])
